...
- **FP32 כברירת מחדל ל-baseline** (במקום FP16): נבחר במקור כדי למקסם את הלחץ על
  ה-RAM. **עדכון (v1.02)**: לאחר תיקון מדידת ה-RAM (~7.65GB בפועל, לא 76GB),
  גם FP16 (~14GB) לא נכנס לזיכרון — כך שגם FP16 כברירת מחדל היה מדגים כישלון.
  FP32 נשאר ברירת המחדל להמשכיות התיעוד; FP16 זמין כאופציית קונפיגורציה לניסוי
  משני (השוואת precision בנוסף לשיטת טעינה).

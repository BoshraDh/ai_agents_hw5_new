...
### ✅ ניסוי 5 — ניתוח כלכלי: On-Prem מול API (חובה, ex05 §5.5)

**מטרה:** לחשב מתי כדאי כלכלית להריץ מקומית לעומת שימוש ב-API חיצוני, עבור אותה
משימה בדיוק. **בסיס החישוב**: זמן ריצה אמיתי שנמדד בניסוי 4 (הקוונטיזציה —
השיטה המקומית המהירה ביותר שהצליחה, 585.9 שניות לבקשה).
